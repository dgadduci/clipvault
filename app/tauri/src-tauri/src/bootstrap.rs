...
mod tests {
    ...
    // -----------------------------------------------------------------
    // `gnome-wayland-integration` regression pin:
    //
    // The package's `[features]` table MUST keep
    // `linux-gnome-shell-integration` in the `default` array so a plain
    // `cargo tauri dev` on Ubuntu GNOME Wayland compiles the real
    // commands / `gnome_integration.rs` instead of the
    // `feature_disabled` stubs in `commands.rs`. The user observed
    // the option greyed out with `reason: "feature_disabled"` because
    // the package-level feature was off even though the target-specific
    // Linux dependency block passed the feature through to
    // `clipvault-platform`. Enabling the feature on the dependency
    // without enabling it on the package itself leaves the
    // `cfg(all(target_os = "linux", feature = "linux-gnome-shell-integration"))`
    // arms in the shell `false` at compile time.
    //
    // The test parses the shell `Cargo.toml` directly through the same
    // minimal TOML walker the other manifest regressions use, so the
    // assertion runs on every developer machine (including the macOS
    // host the dev workflow already exercises) and never depends on
    // Ubuntu or a real GNOME session.
    // -----------------------------------------------------------------

    /// Structural regression: the shell `[features].default` table
    /// MUST include `linux-gnome-shell-integration`. The bug surfaced
    /// as a disabled "Integración GNOME Wayland" toggle returning
    /// `reason: "feature_disabled"` because the cfg arms in
    /// `main.rs`, `lib.rs`, `commands.rs` and `bootstrap.rs` all
    /// resolved to `false` during a normal `cargo tauri dev` build on
    /// Ubuntu. The fix promotes the feature to the package's
    /// `default` array; this test guards the promotion so a future
    /// rebase cannot silently drop it again.
    #[test]
    fn shell_linux_gnome_shell_integration_feature_is_in_default() {
        let manifest_path = std::path::PathBuf::from(env!("CARGO_MANIFEST_DIR")).join("Cargo.toml");
        let source = std::fs::read_to_string(&manifest_path)
            .unwrap_or_else(|error| panic!("read shell Cargo.toml: {error}"));
        let defaults = shell_default_features(&source)
            .expect("shell Cargo.toml must declare a [features] table with default = [...]");
        assert!(
            defaults
                .iter()
                .any(|feature| feature == "linux-gnome-shell-integration"),
            "shell [features].default must include `linux-gnome-shell-integration` so cargo tauri dev on Ubuntu GNOME Wayland compiles the real commands; otherwise `feature_disabled` reaches the user. defaults = {defaults:?}"
        );
    }

    /// Locate the `[features]` table in the shell `Cargo.toml` and
    /// return the `default = [ ... ]` array as a list of trimmed
    /// strings. Returns `None` when the `[features]` header or the
    /// `default` array is missing so the caller can panic with a
    /// descriptive message instead of indexing into an empty vector.
    ///
    /// The walker mirrors the targeted, hand-rolled style the other
    /// manifest regressions use: the shell `Cargo.toml` has a single
    /// `[features]` table and a single `default = [...]` array, so a
    /// full TOML dependency would add build-graph weight without
    /// strengthening the contract.
    fn shell_default_features(source: &str) -> Option<Vec<String>> {
        ...
        let section_body: String = rest
            ...
            .collect::<Vec<_>>()
            .join("\n");
        // Find the literal `default = [ ... ]` array. We only need the
        // substring between `[` and `]` and ignore anything that may
        // follow on the same line.
        let features_marker = "default = [";
        let feat_open = section_body.find(features_marker)? + features_marker.len();
        let feat_close = section_body[feat_open..].find(']')? + feat_open;
        let raw = &section_body[feat_open..feat_close];
        Some(
            raw.split(',')
                .map(str::trim)
                .map(|item| item.trim_matches('"').to_string())
