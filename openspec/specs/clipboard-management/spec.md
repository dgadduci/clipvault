...
Definir la gestión mínima del historial: favoritos, eliminación y expiración configurable.

## Requirements

### Requirement: Manage favorites

ClipVault SHALL allow users to mark and unmark any text history entry as a favorite, and favorites SHALL be ranked ahead of non-favorites when other ranking factors are comparable.

#### Scenario: Mark an entry favorite

- **WHEN** the user marks a history entry as favorite
- **THEN** the favorite state is persisted locally and is reflected in history and search results

#### Scenario: Unmark an entry favorite

- **WHEN** the user removes the favorite state
- **THEN** the entry remains in history with `is_pinned` set to false

### Requirement: Favorites do not expire automatically

Favorite entries SHALL be excluded from automatic expiration while their favorite state remains active.

#### Scenario: Expiration job encounters a favorite

- **WHEN** the configured retention period is reached for a favorite entry
- **THEN** the entry remains available until the user explicitly deletes or unpins it
...
ClipVault SHALL provide a local history retention setting with the options 7 days, 30 days, 90 days and forever, and SHALL apply it to non-favorite text and
image entries. Image assets SHALL be collected only after confirming that no
remaining history row references them.

#### Scenario: Retention removes expired entries

- **WHEN** the application performs retention cleanup and a non-favorite entry is older than the configured period
- **THEN** the entry is removed from the database and search index
...
#### Scenario: Forever retention

- **WHEN** the user selects forever
- **THEN** automatic retention cleanup does not remove non-favorite entries
