...
Definir cómo ClipVault permite a un par vinculado y activo importar una captura de imagen remota sólo cuando el usuario activa Importar sobre una fila remota elegible: la transferencia es autenticada por mTLS, el asset se valida y se persiste localmente, la operación es idempotente por peer, identificador remoto y hash canónico del PNG, y queda asociada a la colección vinculada por peer_id sin exponer referencias remotas, rutas, hashes ni bytes de la imagen en errores o eventos.
