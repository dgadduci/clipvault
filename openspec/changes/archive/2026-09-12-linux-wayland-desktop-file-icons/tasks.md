...
- [x] 5.1 Cerrar por decisión explícita la verificación manual con Firefox nativo:
  el entorno disponible no es Ubuntu GNOME Wayland y no se amplía el cambio
  para obtener una matriz manual adicional.
- [x] 5.2 Cerrar por decisión explícita la repetición con Chrome, Terminal GNOME
  y Warp; la cobertura automatizada y el contrato metadata-only quedan
  registrados, sin ampliar este cambio por falta de una sesión objetivo.
- [x] 5.3 Cerrar por decisión explícita la verificación manual de una app
  window-backed; el contrato automatizado de `window:*` y el fallback seguro
  permanecen como evidencia del cambio.
- [x] 5.4 Diferir la selección visual de blacklist en Linux al cambio
  `linux-blacklist-app-picker`. Este cambio conserva el fallback de ingreso
  manual y no implementa el selector visual Linux.
- [x] 5.5 Cerrar por decisión explícita el smoke test manual X11/XWayland; las
  regresiones automatizadas de los matchers existentes permanecen vigentes.
...
El usuario autoriza cerrar este cambio aceptando las tareas manuales como
alcance diferido/documental. La selección visual de blacklist Linux queda
fuera de este cambio y se registra como propuesta independiente en
`linux-blacklist-app-picker`.
