# desktop-shell-layout Specification
...
ClipVault SHALL replace the inline `Clear non-favorite history` button with an
accessible trash icon in the top-right toolbar that reuses the existing
confirmation and `clearHistoryCommand` flow.

#### Scenario: Open clear confirmation from the trash icon

- **WHEN** the user activates the trash icon
- **THEN** the existing confirmation flow opens before any history mutation occurs

#### Scenario: Cancel clear history

- **WHEN** the user cancels the confirmation
- **THEN** no entry, favorite, tag, collection or asset is modified

#### Scenario: Confirm clear history

- **WHEN** the user confirms the trash action
- **THEN** only non-favorite history is cleared through the existing command, favorites remain and the rail refreshes after success

#### Scenario: Clear failure is safe

- **WHEN** the clear command fails or requests confirmation again
- **THEN** the existing error/confirmation state is shown and the toolbar remains usable

#### Scenario: Individual card delete remains

- **WHEN** the user opens a card menu
- **THEN** the existing individual `Delete` action and confirmation flow remain available
