...
ClipVault SHALL capture a supported raster image when the clipboard has no
non-empty text representation, SHALL preserve the existing text-first behavior
when both text and image are available, and SHALL keep unsupported binary
representations out of the history without terminating the application.

#### Scenario: Image without text is copied

- **WHEN** a user copies a supported raster image and the clipboard has no
  non-empty text representation
- **THEN** the capture pipeline creates one image history entry and continues
  running

#### Scenario: Text and image are both available

- **WHEN** the clipboard exposes non-empty text and an image for the same
  operation
- **THEN** ClipVault captures the text using the existing text pipeline and
  does not create an additional image entry for that operation

#### Scenario: Unsupported binary content is copied

- **WHEN** the clipboard exposes a file, audio, video, RTF-only or unsupported
  binary representation without supported text or image data
- **THEN** ClipVault returns an ignored/unsupported outcome, creates no history
  row and keeps the watcher alive
